...
public static class ContextPromptService
{
    ...
    public static string GetPullRequestReviewPrompt(int? pullRequestNumber, string? reviewerLogin)
    {
        ...
        return $"""
            ...
            2. Use `gh pr diff {numberText}` to inspect the proposed changes, then check whether the working tree is clean with `git status --short`. Do not modify files, create branches, push commits or request changes to the pull request; this is a review-only assignment.

            3. Submit a review for the pull request using `gh pr review {numberText}`. If the pull request cannot be reviewed because its state is ambiguous or its changes cannot be verified, stop and report the ambiguity instead of submitting a speculative review.
