...
public static class ContextPromptService
{
    public static string GetNewIssuePrompt(int number)
    {
        return $"""
            ...
            1. If the issue is not clear, please ask for clarification in the comments of the issue. The clarification comment must start with a visible quoted notice, followed by the actual question. Write the notice in the same language as the issue or clarification conversation. In English, use: `> 🤖 This clarification request was generated automatically by a Codex session through CGR. After providing the requested information, transition this issue back to `ready`.` In Turkish, use: `> 🤖 Bu açıklama talebi, CGR üzerinden çalışan bir Codex oturumu tarafından otomatik olarak oluşturulmuştur. İstenen bilgiyi sağladıktan sonra issue'yu tekrar `ready` durumuna geçirin.` Do not make any assumptions about the issue, run `cgr issue transition {number} needs-info` after leaving comment. Wait for the issue author to provide clarification before proceeding.
